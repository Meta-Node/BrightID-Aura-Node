...
- An endpoint that takes longer than 10 seconds to connect, or stalls longer than 10 seconds while answering `clique_status`, is now treated as a failed read. A node pointed at one would log errors instead of waiting, and at startup would not begin applying blocks until a read succeeds.
